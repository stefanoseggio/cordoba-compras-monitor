...
- **Dependency vulnerabilities** in `package.json`'s real dependency tree (`apify`, `cheerio`, `https-proxy-agent`, and dev dependencies) are a real, ongoing concern — tracked via Dependabot (`.github/dependabot.yml`) and GitHub's own dependency/secret scanning, both enabled on this repository.
- **TLS chain handling.** The source's server sends an incomplete certificate chain; the Actor supplies the missing intermediate via `NODE_EXTRA_CA_CERTS`, additively extending Node's trust store, rather than disabling certificate validation. A compromised or spoofed source endpoint is outside this Actor's control.
